...
def create_user_to_verify(api):
    requst_data = {"language": "EN", "email": "verify@gmail.com", "password": "some-password", "username": "username"}
...
def mailhog(response, email: dict, type: str):
    if (response.status_code == 201 or response.status_code == 200):
        ...
        for message in messages:
            ...
            return _find_message(message=message, receivers=receivers, email=email, type=type)
    return None
...
def _find_message(message, receivers, email, type: str = "code"):
    for receiver in receivers:
        if (receiver["Mailbox"] == email["box"] and receiver["Domain"] == email["domain"]):
            if type == "code":
                return _get_code(message=message)
            else: return _get_url(message=message)
    return None
